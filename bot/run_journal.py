...
всего: 01.08 прогон уехал с слота 00:15 на 11:38 — машину выключили («Завершение
работы», выполненное как гибернация ядра при включённом Быстром запуске; долг
№45). К моменту проверки бар был на месте, и сторож сказал «✅ Форвард жив».
Долг №46.
...
состояния, которые требуют РАЗНЫХ действий:
    ...
    нет start           → не запускался (выключение, сон, задача отключена)
