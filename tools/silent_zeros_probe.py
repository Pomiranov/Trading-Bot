...
🚩 ГРАНИЦА ИНСТРУМЕНТА. Читать ДО того, как предъявлять его вывод как замер.

Предикат различает НАЛИЧИЕ умолчания и НЕ различает НАПРАВЛЕНИЕ ответа.
Разрешительный False фильтра («даунтренда нет — не блокируй») и запретительный
False условия («условие не выполнено — сигнала нет») для него неразличимы.
Поэтому вывод — КАНДИДАТЫ, а не находки: на прогоне 02.08 их было 529 на 155
файлах, а разрешительных на пути решения после разбора осталось девять.
Направление назначается чтением, и этот шаг пропустить нельзя.

Чего предикат НЕ ловит — показано фактом 02.08, обе находки получены чтением:

  - разрешение, выраженное НЕПУСТОЙ структурой:
        return {"approved": True, "confidence": Decimal("0.5")}
    (trading_orchestrator.py:191-205 — неизвестная стратегия разрешается);
  - нелитеральное нейтральное умолчание:
        confidence = strategy_stats["confidence"] or Decimal("0.5")
    (там же, :207 — Decimal(...) это вызов, а не константа);
  - ОТСУТСТВУЮЩУЮ проверку: пропущенного условия в AST нет по построению.

Ещё одна граница, найденная там же: «достижимость файла импортом» и «исполнение
функции» — РАЗНЫЕ предикаты. Колонка «живой» считается по первому. Из-за этого
две находки в risk_manager.py получили завышенную тяжесть: файл импортируется
форвардом, а функции зовутся только из main.py и gateway. Исполнение
доказывается артефактом — строкой в БД или в логе, — а не графом импортов.

Область проверки задаётся предикатом достижимости от точки входа, а НЕ списком
каталогов: список, которым область задавалась 02.08, увёл главный файл в «вне
области» вместе с 63 живыми срабатываниями (правило 14 §8).

Правило 12 §8: прежде чем предъявлять ноль, прогнать на подсадке. Образец —
12 положительных случаев по два на каждый вид плюс отрицательные контроли
(fillna(True), обязательный ключ, raise вместо return, непустые умолчания).
Ноль без демонстрации ловли не принимается.

Ничего не пишет в проект и ничего из него не импортирует.
Запуск:  python tools/silent_zeros_probe.py <корень репозитория>
"""
import ast
import sys
