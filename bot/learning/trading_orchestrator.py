...
# Канон меток и сессий — одна копия на проект (долг №16). Нужен запасному пути
# границы фильтра в _check_structural_downtrend (долг №48, форма II).
from market_time import MSK, d1_bar_time, session_date
...
class TradingOrchestrator:
    ...
    async def _check_structural_downtrend(self, signal: dict) -> Optional[str]:
        ...
        # ── ДОЛГ №48, ФОРМА II: граница внутри запроса фильтра ────────────
        #
        # ДО 2026-08-03 запрос шёл БЕЗ верхней границы, а `is_structural_downtrend`
        # берёт `.iloc[-1]` полной серии. Значит фильтр считался по САМОЙ СВЕЖЕЙ
        # строке таблицы, а не по бару решения, и совпадал с бэктестом
        # (`dt_gate.iloc[i]`) только когда это одна и та же строка. У форварда
        # редко, у живого контура НИКОГДА: `main.py:222` передаёт timeframe "1h",
        # а здесь жёстко `timeframe='1d'`, то есть последняя дневная строка у него
        # всегда формирующийся бар сегодняшнего дня.
        #
        # `filter_until` — ИСКЛЮЧАЮЩАЯ верхняя граница на `candles.time`, равная
        # НАЧАЛУ СЕССИИ, В КОТОРОЙ ПРИНИМАЕТСЯ РЕШЕНИЕ. Одно понятие покрывает оба
        # контура (таблица §6):
        #   форвард — решение принимается ПОСЛЕ закрытия сессии D, значит граница
        #             = начало сессии D+1, и сессию D фильтр видит (верно: бар
        #             решения и есть дневной бар сессии D);
        #   живой   — решение принимается ВНУТРИ сессии D, граница = начало D,
        #             формирующийся дневной бар отсекается, фильтр видит D−1.
        #
        # ⚠ ПРИНЦИП: граница идёт ОТ БАРА РЕШЕНИЯ. Значение из `now()` — запасной
        # путь для вызывающего, который бара не передаёт (сегодня это только
        # `main.py`, и он не правится). Запасной путь ПЕРЕСТАЁТ БЫТЬ ВЕРНЫМ, если:
        #   (а) входы разрешат на догоняемых барах ФАЗЫ 1 форварда — тогда бар
        #       решения старше `now()`, и граница по `now()` вернёт заглядывание;
        #   (б) прогон пойдёт под FWD_REPLAY на отмотанных данных — `now()` к
        #       данным не относится вовсе (см. `run_forward_d1.py:184-191`). Это
        #       наступает уже в парном контроле долга №48, а не когда-нибудь;
        #   (в) слот переедет так, что сессия бара решения перестанет быть
        #       предыдущей относительно сессии прогона.
        # Поэтому вызывающий, у которого бар решения ЕСТЬ, обязан передать границу.
        until = signal.get("filter_until")
        if until is None:
            until = d1_bar_time(session_date(datetime.now(MSK)))

        async with self._pool.acquire() as conn:
            rows = await conn.fetch("""
                SELECT time, close FROM candles
                WHERE ticker = $1 AND timeframe = '1d' AND time < $2
                ORDER BY time
            """, ticker, until)
