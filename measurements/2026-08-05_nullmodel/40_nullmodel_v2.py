...
def drift_control(tag):
    """Контроль. ПОЧИНЕН 2026-08-05 по правилу 24 §8.

    Прежняя редакция отвечала на ДВА вопроса одним числом и при отказе не говорила,
    на какой именно: (1) дрейфнули ли данные ПОСРЕДИ серии — сравнение "до" против
    "после"; (2) воспроизводится ли опорное число — сравнение с заморозкой. Отказ
    пришёл по (2) и прочитался как отказ (1).
    Причина отказа (2): сравнивалась СЫРАЯ сумма движка с целью, выведенной из
    ОКРУГЛЁННЫХ значений CSV, а сумма округлённых не равна округлению суммы.
    Починка — СВЕДЕНИЕ СПОСОБА, не ослабление допуска: печатаются ОБЕ суммы, каждая
    сверяется со своей целью, посчитанной ТЕМ ЖЕ способом. Допуск не введён — он
    скрыл бы и настоящий дрейф.
    """
    ...
    s_raw = sum(out)
    s_round = sum(round(x, 2) for x in out)
    ok_shape = (n == 18 and round(100*w/n, 1) == 72.2 and round(pf(out), 3) == 1.638)
    ok_raw = round(s_raw, 2) == 127747.96
    ok_round = round(s_round, 2) == 127747.97
    line = (f"[{tag}] n={n} WR={100*w/n:.1f}% PF={pf(out):.3f} "
            f"PnL_syraya={s_raw:.2f} PnL_okrugl={s_round:.2f}")
    print("  " + line)
    print(f"       vosproizvodimost: syraya {'OK' if ok_raw else 'X'} 127747.96, "
          f"okrugl {'OK' if ok_round else 'X'} 127747.97, forma {'OK' if ok_shape else 'X'}")
    return dict(ok=ok_shape and ok_raw and ok_round, raw=s_raw, rnd=s_round, line=line)
...
print("KONTROL DO SERII:")
c_before = drift_control("do")
...
print("KONTROL POSLE SERII:")
c_after = drift_control("posle")
drift_ok = (c_before["raw"] == c_after["raw"] and c_before["rnd"] == c_after["rnd"])
print()
print("VERDIKT 1, DREYF (do protiv posle): " + ("NET DREYFA" if drift_ok else "DANNYE IZMENILIS POSREDI SERII - SERIYA NEDEYSTVITELNA"))
print("VERDIKT 2, VOSPROIZVODIMOST zamorozki: " + ("SOSHLOS" if (c_before["ok"] and c_after["ok"]) else "RASHOZHDENIE"))
...
Path(OUT/"25_SUMMARY.json").write_text(json.dumps(dict(
    ...
    median_top2_share=st.median(shares), drift_before=c_before["line"], drift_after=c_after["line"],
    drift_ok=bool(drift_ok), reproducible=bool(c_before["ok"] and c_after["ok"])), ensure_ascii=False, indent=2), encoding="utf-8")
